...
export default withAuth(async (req: VercelRequest, res: VercelResponse, ctx: ApiContext) => {
  ...
  // Free tier defaults to 20 wallets for faster response
  ...
  try {
    ...
    // Set longer cache for heavy endpoint
    res.setHeader('Cache-Control', 's-maxage=300, stale-while-revalidate=600');

    res.status(200).json({ ok: true, data: result });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Smart money scan failed';
